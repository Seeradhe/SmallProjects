 ...
 * 				 > versucht diesen Fehler weiter zu verbessern, ohne erfolg
 */
...
int main ()
{
	...
	kathetenverhaltnis  = ((1. * rand() - minzz) / (maxzz - minzz) 
		* (maxf - minf) + minf);
...
void Dreieck (double X1,double Y1,double X2,double Y2,double c,double Alpha,int Tiefe1)
{
	...
	printf("c: %lf | b: %lf | sin rechnung: %lf\n", c, b, sin(atan(kathetenverhaltnis)));
	Alpha = Alpha + (asin(b/c)*180./M_PI); 	//hier wieder im Gradmaß
	
	c = sqrt (c*c - b*b); //berechnung ankate
	printf("a: %lf", c);
	XHilf = X1 - (c * sin(((90.-Alpha)*M_PI)/180.));
	YHilf = Y1 + (c * sin(Alpha*M_PI/180.));
	
	printf("DWert Alpha: %lf \n", Alpha);
	...
	Quader2 (XHilf, YHilf, X2, Y2, b, Alpha , Tiefe1-1);
}
